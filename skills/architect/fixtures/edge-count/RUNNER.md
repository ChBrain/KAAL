<!-- generated by kaal runner architect edge-count; edit the skill or the fixture, then regenerate -->

# Runner: architect on edge-count

Three blocks, each with a copy button on the repository page. Send the first as one message in a fresh conversation, in the setup the record will name. Paste the whole exchange, first line to last, your own answers included, after the second block's last line and send it as a fresh message. Fill the third block in, then the two sections, and file it as `evals/architect/edge-count/<model>.md`.
...
Draw the space for this requirement, three criteria, three red acceptance
tests, all about a `report` command:

1. `report --json` prints one JSON object per run.
2. `report --json` exits 2 when the input file is missing.
3. `report` with no flag prints the table it printed before.
````
...
Checklist:
- The drawing lists three numbered seams, and no seam that no criterion
  needs.
- The mermaid block carries three labelled edges, numbered 1 to 3 to match
  the seam list.
- There are three contract tests, numbered to match the seams, each blind to
  the code behind its seam.
- The test strategy table has a row for every one of the three criteria.
- The Fixed and free section fixes the JSON shape and the exit code, and
  leaves the table's rendering free.
