<!-- generated by kaal runner analyse vacuous-loop; edit the skill or the fixture, then regenerate -->

# Runner: analyse on vacuous-loop

Three blocks, each with a copy button on the repository page. Send the first as one message in a fresh conversation, in the setup the record will name. Paste the whole exchange, first line to last, your own answers included, after the second block's last line and send it as a fresh message. Fill the third block in, then the two sections, and file it as `evals/analyse/vacuous-loop/<model>.md`.
...
Every agent under `agents/` must have an `AGENT.md` whose frontmatter names
a `lane`. Write the requirement and its acceptance test. Right now `agents/`
is empty; the first agent lands next week.
````
...
date: <YYYY-MM-DD>
fixture: vacuous-loop
ask_sha: 1e163e5ca0fe4a536788b3beca36c75b30e50da0816287b2ac9dfcb0aec474d9
expect_sha: 4ff3f9c82e867eb7a2e118d04e2a09bb002a5d5e789ed7314105094c5758b3c4
