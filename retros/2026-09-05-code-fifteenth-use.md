# Retrospective: 4 L's

Self-diagnosis: drawn from session context. Not a human-reported retro.

Period: the fifteenth use of the code skill, on `architecture/code-v2`,
5 September 2026.

## Liked

- The board shows counts on every wall now: `acceptance (84 passing)`,
  `contracts (44 passing)`. A number that was asked for twice in the
  retros cost one line in the runner and one in each command.
- The standings line shows the league where its evidence stands: the
  analyse candidates read `1 of 2` from the one hand-made record.

## Learned

- `kaal fixtures` lists sixty-odd artefacts under fixtures directories;
  that is the surface a new rule reaches, and now it can be read before
  the board finds it.

## Lacked

- Nothing new.

## Longed for

- The second record, so a standing reads `2 of 2` and a move climbs.

Feeds: `code`.
