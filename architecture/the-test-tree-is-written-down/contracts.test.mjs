...
// The same, narrowed to what this wall says about plans. Seams 2 to 4 read
// only their own findings, so a break in the place beneath them reddens seam
// 1 and not all four: an isolation that cannot fall alone answers nothing.
const asPlan = (out, name) =>
  about(out, name).filter((l) => /: plan: /.test(l));
const namesPlan = (out, name, what) =>
  assert.ok(
    asPlan(out, name).length,
    `no plan finding names ${name} (${what}): ${out}`,
  );
const silentPlan = (out, name, what) =>
  assert.deepEqual(
    asPlan(out, name),
    [],
    `${name} was a plan finding and should not be (${what})`,
  );
...
test("1. tests/ is a place: its pages are artefacts, and a parent inside it resolves to a page", () => {
  ...
  const out = said(r);
  notUsage(out);
  // The listing: a page directly under tests/ is read, a page under a
  // subdirectory of it is not, and a file that is not .md is not. `dangling`
  // is a plan whose wall exists and whose pin is right, so the only thing
  // that can be said about it is that its parent resolves to nothing.
  assert.equal(r.status, 1, `an unresolvable parent was allowed: ${out}`);
  names(out, "dangling", "a page directly under tests/");
  silentAbout(out, "buried", "a page under a subdirectory of tests/");
  silentAbout(out, "notes", "a file under tests/ that is not .md");
  // And where a name inside the place resolves. `acceptance` declares the
  // strategy as its parent and the strategy is a page of this place, so a
  // finding here is a place that resolved a name to a directory beneath the
  // page rather than to the page.
...
test("2. a plan naming a wall the config does not hold is a finding naming the plan", () => {
  ...
  namesPlan(out, "orphaned", "the plan whose wall does not exist");
  ...
  silentPlan(out, "acceptance", "the plan whose wall exists");
  ...
  namesPlan(noneOut, "wordless", "the plan that names no wall");
  silentPlan(noneOut, "acceptance", "the plan that names one");
});
...
test("3. a test gate no plan names is a finding naming the wall, and a gate that runs no tests owes nothing", () => {
  ...
  namesPlan(out, "acceptance", "the test gate no plan names");
  silentPlan(out, "format", "a gate whose command runs no tests");
  ...
  namesPlan(twoOut, "acceptance", "the wall two plans name");
});

test("4. a plan's suites are its wall's suites, pinned to what they match", () => {
  const wrong = kaal("traces", F("count-wrong"));
  const out = said(wrong);
  notUsage(out);
  assert.equal(wrong.status, 1, `a pin that disagrees was allowed: ${out}`);
  const finding = asPlan(out, "acceptance").join("\n");
  assert.ok(finding, `no plan finding names the plan: ${out}`);
  assert.match(finding, /\b7\b/, `the number pinned is not said: ${finding}`);
  assert.match(finding, /\b1\b/, `the number found is not said: ${finding}`);
  // A plan pinned correctly to globs that are not its wall's is the other
  // half of the seam, and it says so in different words: the pin is right
  // and the plan is about something else.
  ...
  namesPlan(otherOut, "contracts", "the glob the plan carries");
});
