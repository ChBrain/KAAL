...
test("1. template to handoff: five lines in order, the last on one line, and a stamped requirement reads closed", () => {
  const h = section(
    readFileSync(
  ...
  assert.ok(!people.endsWith(","), "the People placeholder wraps");
...
test("3. a rule at its seat: the analyse section in its place with its number, the two paragraphs in theirs, every skill within the rules", () => {
  ...
  assert.match(folded(section(a, "Data about a person")), /system of record/);
